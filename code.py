        ...
        for l in lines:
            ...
            if flag:
                if (end== tokens[0] and (len(tokens)==1 or tokens[1][0] =='#')):
                    flag = 0
                contains.append(l)
            if(ac_check == tokens[0]  and tokens[1][0] == 'V'):
                ac_flag = 1
                w = parse_val(tokens[2])
                print('Frequency :{}'.format(w),'number of ac sources:{}'.format( ac_flag))
                w = w* 2*math.pi
                break
        if(len(contains)==0):
...
i=0
for n in nodes:
    print("Voltage at Node " + n+" = {} " .format( X[i]))
    i = i+1
for V in voltage_sources:
    print("Current Through Voltage Source " + V.name + " ={} ".format(X[i]))
    i= i+1
